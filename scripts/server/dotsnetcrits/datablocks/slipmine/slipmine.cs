//-----------------------------------------------------------------------------
// slipMine weapon. This file contains all the items related to this weapon
...
// is mounted onto a shape, the slipMineImage is used.
...
datablock ExplosionData(slipMineExplosion)
{
   soundProfile = slipMineExplosionSound;
...
datablock ProximityMineData(slipMine)
{
   // Mission editor category
   category = "Weapon";
   explosion = slipMineExplosion;
   ...
   explosionOffset = 0.1;
    ...
    pickUpName = "a slipMine";
    description = "slipMine";
    maxInventory = 1;
...
//--------------------------------------------------------------------------
// slipMine image which does all the work.  Images do not normally exist in
...
datablock ShapeBaseImageData(slipMineImage)
{
   ...
   //imageAnimPrefix = "slipMine";
   //imageAnimPrefixFP = "slipMine";
   ...
   stateSound[3]                    = slipMineFireSound;
   ...
   //stateSound[4]                    = slipMineReloadSound;
   ...
   //stateSound[6]                    = slipMineFireEmptySound;
};
...
function slipMine::onUse(%this, %obj)
{
   // Act like a weapon on use
   Weapon::onUse( %this, %obj );
}

function slipMine::onPickup( %this, %obj, %shape, %amount )
{
   // Act like a weapon on pickup
   Weapon::onPickup( %this, %obj, %shape, %amount );
}

function slipMine::onInventory( %this, %obj, %amount )
{
   %obj.client.setAmmoAmountHud( 1, %amount );

   if (!%amount)
   {
     %obj.setInventory(slipMine, 1);
     return;
   }
...
function slipMineImage::onMount( %this, %obj, %slot )
{
...
function slipMineImage::onUnmount( %this, %obj, %slot )
{
   %obj.client.RefreshWeaponHud( 0, "", "" );
}

function slipMineImage::onFire( %this, %obj, %slot )
{
   // To fire a deployable mine is to throw it
   %obj.throw( %this.item );
}

function slipMine::onExplode( %this, %obj, %position )
{
  initContainerRadiusSearch(%position, %this.damageRadius, $TypeMasks::ShapeBaseObjectType);

  while ( (%objTarget = containerSearchNext()) != 0 )
  {
    %objTarget.applyImpulse("0 0 0", "0 0 500");
    %velocity = %objTarget.getVelocity();
    %velocity = VectorScale(%velocity, 10);
    %objTarget.setVelocity(%velocity);
  }
}

DefaultPlayerData.maxInv[slipMine] = 1;
