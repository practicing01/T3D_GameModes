//-----------------------------------------------------------------------------
// singlecell weapon. This file contains all the items related to this weapon
...
// is mounted onto a shape, the singlecellImage is used.
...
datablock ItemData(singlecell)
{
   ...
   shapeFile = "art/shapes/dotsnetcrits/weapons/singlecell/singlecell.cached.dts";
   mass = 1;
   elasticity = 0.2;
   friction = 0.6;
   emap = true;
    ...
    pickUpName = "a singlecell";
    description = "singlecell";
    maxInventory = 1;
    damageType = "meleeDamage";
    damageRadius = 10;
    directDamage = 1000;
    image = singlecellImage;
    reticle = "crossHair";
...
//--------------------------------------------------------------------------
// singlecell image which does all the work.  Images do not normally exist in
// the world, they can only be mounted on ShapeBase objects.

datablock ShapeBaseImageData(singlecellImage)
{
   // Basic Item properties
   shapeFile = "art/shapes/dotsnetcrits/weapons/singlecell/singlecell.cached.dts";
   //shapeFileFP = "art/shapes/dotsnetcrits/weapons/singlecell/singlecell.cached.dts";
   emap = false;
   ...
   //imageAnimPrefix = "singlecell";
   //imageAnimPrefixFP = "singlecell";
   ...
   //stateSound[3]                    = singlecellFireSound;
   ...
   //stateSound[4]                    = singlecellReloadSound;
   ...
   //stateSound[6]                    = singlecellFireEmptySound;
};


//-----------------------------------------------------------------------------

function singlecellImage::onMount( %this, %obj, %slot )
{
...
function singlecellImage::onUnmount( %this, %obj, %slot )
{
   %obj.client.RefreshWeaponHud( 0, "", "" );
}

function singlecellClass::ProcessAI(%this)
{
  singlecellAI.onReachDestination(%this);

  %this.aiSchedule_ = %this.schedule(1 * 1000, "ProcessAI");
}

function singlecellImage::Spawnsinglecell( %this, %obj, %slot )
{
   %pos = %obj.getPosition();
  ...
  %singlecell = new AiPlayer()
  {
    dataBlock = singlecellAI;
    class = singlecellClass;
    //mMoveTolerance = 1.0;
  ...
  %singlecell.rotation = %obj.rotation;

  %sizeTarget = %singlecell.getObjectBox();
  %scaleTarget = %singlecell.getScale();
  %sizexTarget = (getWord(%sizeTarget, 3) - getWord(%sizeTarget, 0)) * getWord(%scaleTarget, 0);
  %sizexTarget *= 1.5;

  %singlecell.position = VectorAdd( %pos, VectorScale(%teleDir, %sizex + %sizexTarget) );
...
function singlecellImage::onFire(%this, %obj, %slot)
{
  %this.schedule(0, "Spawnsinglecell", %obj, %slot);//spawning within causes crash
...
function singlecellAI::onReachDestination(%this, %ai)
{
...
function singlecellAI::onMoveStuck(%this, %ai)
{
  if (!isObject(%ai.target_))
  {
    //%ai.target_ = ClientGroup.getRandom().getControlObject();
...
function singlecellAI::onDisabled(%this, %obj, %state)
{
  initContainerRadiusSearch(%obj.position, 10.0, $TypeMasks::ShapeBaseObjectType);

  while ( (%targetObject = containerSearchNext()) != 0 )
  {
    //if(%targetObject != %obj)
    //{
     %targetObject.damage(%obj, %pos, 1000, "singlecell");
...
function singlecellClass::AttackCD(%this)
{
  %this.canAttack_ = true;
}

function singlecellAI::onCollision(%this, %obj, %collObj, %vec, %len)
{
...
DefaultPlayerData.maxInv[singlecell] = 1;
