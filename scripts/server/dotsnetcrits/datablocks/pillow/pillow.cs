//-----------------------------------------------------------------------------
// pillow weapon. This file contains all the items related to this weapon
...
// is mounted onto a shape, the pillowImage is used.

datablock SFXProfile(pillowFireSound)
{
...
datablock ItemData(pillow)
{
   ...
   shapeFile = "art/shapes/dotsnetcrits/weapons/pillow/pillow.cached.dts";
   mass = 1;
   elasticity = 0.2;
   friction = 0.6;
   emap = true;
...
//--------------------------------------------------------------------------
// pillow image which does all the work.  Images do not normally exist in
...
datablock ShapeBaseImageData(pillowImage)
{
   // Basic Item properties
   shapeFile = "art/shapes/dotsnetcrits/weapons/pillow/pillow.cached.dts";
   //shapeFileFP = "art/shapes/dotsnetcrits/weapons/pillow/pillow.cached.dts";
   emap = false;
   ...
   //imageAnimPrefix = "pillow";
   //imageAnimPrefixFP = "pillow";
   ...
   //stateSound[4]                    = pillowReloadSound;
   ...
   //stateSound[6]                    = pillowFireEmptySound;
};


//-----------------------------------------------------------------------------

function pillowImage::onMount( %this, %obj, %slot )
{
...
function pillowImage::onUnmount( %this, %obj, %slot )
{
...
function pillowImage::onFire(%this, %obj, %slot)
{
   ...
   while ( (%targetObject = containerSearchNext()) != 0 )
   {
     if(%targetObject != %obj)
     {
      %targetObject.damage(%obj, %pos, %this.item.directDamage, "pillow");
     }
   }
}

DefaultPlayerData.maxInv[pillow] = 1;
